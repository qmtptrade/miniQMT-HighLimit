...
def calculate_factors(stock):
    '''
    我先直接解释这段条件分支的含义：它本质上是在“样本不足25条”和“样本达到25条”时，用不同的起始价格来计算短期涨幅。这两个分支的区别是“起点价格取哪里”，从而决定你在算哪段时间的涨幅。

当 num < 25 时
Start_price = data[-1][lastPrice]
这时起点就是当前价本身，所以
last_price - Start_price = 0
你的 factor3 基本恒为 False（0 不会 >= 0.03）。
作用是：在缓存数据还不够25条时，不触发这个因子，相当于预热期。

当 num >= 25 时
Start_price = data[-25][lastPrice]
这时起点是“25条记录前的价格”，所以 factor3 在判断：
最近25个tick累计涨幅（分子）相对昨收（分母）是否 >= 3%。

补充一点：当 num == 25 时，data[-25] 就是这25条里的第一条，逻辑是成立的。
'''
    # 开盘涨幅
    if stock not in A.data_cache:
